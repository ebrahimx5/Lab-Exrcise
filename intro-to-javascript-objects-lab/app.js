...
for (let oneGym of game.gyms) {
  console.log(oneGym.difficulty)
...
for (let dragonName of game.party) {
  console.log(dragonName.name)
}
...
for (let startPokemon of pokemon) {
  // console.log(startPokemon)   # very annoying to see it in inspect
  if (startPokemon.starter === true) {
...
*/
game.catchPokemonUpdate = pokemonObj => {
  game.party.push(pokemonObj)
  for (let updatePokeball of game.items)
    if (updatePokeball.name === 'pokeball') {
      updatePokeball.quantity--
    }
}

game.catchPokemonUpdate(pokemon[35])
game.catchPokemonUpdate(pokemon[78])
game.catchPokemonUpdate(pokemon[90])
game.catchPokemonUpdate(pokemon[104])
console.log(game.party)
...
for (let changeDef of game.gyms) {
  console.log(changeDef.difficulty)
  if (changeDef.difficulty < 6) {
    changeDef.completed = true
  }
}
...
game.gymStatus = function () {
  ...
  // anthor nicer method 
  console.log(` complete gym : ${gymTally.complete} uncomplete gym : ${gymTally.incomplete}`)
}
...
game.partyCount = function() {
  ...
  // if we want show it to user we subtract 1 from the result because programmer language start from zero as anumber and humen language start from 1 
